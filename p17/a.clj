...
(defn neighborhood
    {:test #(do
        (is (= 27 (count (neighborhood [0 0 0]))))
    )}
    [[x0 y0 z0]]
        (for [x (range -1 2) y (range -1 2) z (range -1 2)] [(+ x0 x) (+ y0 y) (+ z0 z)])
)
...
(defn conway
    {:test #(do
        (is (= 11 (conway example 1)))
        (is (= 21 (conway example 2)))
        (is (= 38 (conway example 3)))
        (is (= 112 (conway example 6)))
    )}
    [lines cycles]
    (count
